...
def JEBF_NonAdjacentExactMatches_04_MM_O_O_bifurcate(lsPatterns,ruleSequence):
    ...
    try:
      ...
      Fordebit = fin_L1_TMP_JEBifurcation\
      ...
      Forcredit = fin_L1_TMP_JEBifurcation\
      ...
      un1 = L1_TMP_JEBifurcation_NonAdjacentExactMatch.alias('trans')\
             .join(fin_L1_TMP_JEBifurcation.alias('drRow'),\
      ...
      un2 = L1_TMP_JEBifurcation_NonAdjacentExactMatch.alias('trans')\
             .join(fin_L1_TMP_JEBifurcation.alias('crRow'),\
             ...
             .select(col('crRow.journalSurrogateKey')\
                  ,col('crRow.transactionIDbyPrimaryKey')\
                  ,col('crRow.transactionID')\
                  ,col('crRow.lineItem')\
                  ,col('transactionLineID')\
                  ,col('debitCreditCode')\
                  ,col('accountID')\
                  ,col('crRow.debitAmount')\
                  ,col('crRow.creditAmount')\
                  ,col('transactionLine')\
                  ,col('trans.patternID')\
                  ,col('journalId')\
                  ,col('trans.debitAmount').alias('amount')\
      ...
      fin_L1_STG_JEBifurcation_Link = unionresult\
             .select(col('journalSurrogateKey')\
              ...
              ,lit('33-NonAdjacentExactMatch').alias('rule')\
              ,lit('04-MM-O-O').alias('bifurcationType')\
              ,col('transactionLine')\
              ,col('journalId')\
