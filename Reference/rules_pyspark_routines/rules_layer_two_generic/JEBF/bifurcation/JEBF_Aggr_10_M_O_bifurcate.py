...
def JEBF_Aggr_10_M_O_bifurcate(lsPatterns):
    ...
    try:
      ...
      debit_RowNum = Window.partitionBy("transactionID","patternID").orderBy("transactionLineIDKey") 
      ...
      df1 = df_debit.alias('main')\
              ...
              (col('main.transactionID') == col('sub.transactionID'))&(col('main.patternID') ==col('sub.patternID')),'inner')\
      ...
      fin_L1_STG_JEBifurcation_Link = dfoutput.alias('tTab')\
            ...
            .filter((col('jeb1.patternID').isin(*lsPatterns))\
                    & (col('dw.debitCount') > lit(1)) \
                    & (col('dw.creditCount') == lit(1)))\
            .select(col('tTab.journalSurrogateKey').alias('journalSurrogateKey')\
             ...
             ,lit('10-Agg-M-O').alias('bifurcationType')\
             ,col('jeb1.journalId')\
