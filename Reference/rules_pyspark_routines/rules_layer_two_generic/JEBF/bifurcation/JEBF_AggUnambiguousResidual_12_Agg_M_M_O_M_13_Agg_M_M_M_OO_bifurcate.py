...
def JEBF_AggUnambiguousResidual_12_Agg_M_M_O_M_13_Agg_M_M_M_OO_bifurcate(lsPatterns,ruleSequence):
  
    try:
      objGenHelper = gen_genericHelper()
      
      ommocte = fin_L1_TMP_JEBifurcation_Accountwise_Agg.groupBy("transactionID","patternID")\
                .agg(
      ...
      fin_L1_TMP_JEBifurcation_OMMO = ommocte.alias('cte')\
                .filter( ((col('cte.debitCount') == 1) \
                          & (col('cte.creditCount') > 1)  | (col('cte.creditCount') == 1) \
                          & (col('cte.debitCount') > 1))\
                          & (col('cte.patternID').isin(*lsPatterns)))\
                .select(col('cte.transactionID')\
                 ,col('cte.creditCount')\
                 ,col('cte.debitCount')\
                 ,col('cte.patternID')\
                 ,when((col('cte.debitCount') == 1) \
                         & (col('cte.creditCount') > 1) ,lit('12-Agg-MM-O-M'))\
                        .otherwise(lit('13-Agg-MM-M-O')).alias('bifurcationtype'))

      df_cte = fin_L1_TMP_JEBifurcation_Accountwise_Agg.alias('oneside')\
              .join(fin_L1_TMP_JEBifurcation_OMMO.alias('ommo'),\
                 (col('oneside.transactionID') ==	col('ommo.transactionID')) \
                   ...
                   & (((col('oneside.debitCreditCode') == lit('D')) \
                         & (col('ommo.debitCount') == 1)) \
                       | (col('oneside.debitCreditCode') == lit('C')) \
                       & (col('ommo.creditCount') == 1))\
                   ,how =('inner'))\
      ...
      fin_L1_TMP_JEBifurcation_UnambiguousBifurcated = df_cte.alias('oneside')\
            ...
            .select(col('oneside.transactionID')\
             ,col('oneside.transactionLineID')\
             ,col('oneside.transactionLineIDKey')\
             ,concat(col('oneside.transactionLineID'),lit('.'),F.row_number().over(dr_line)).alias("lineItemNumber")\
      ...
      fin_L1_STG_JEBifurcation_Link = fin_L1_TMP_JEBifurcation_Accountwise_Agg.alias('jeb1')\
            ...
            .select(col('jeb1.journalSurrogateKey')\
             ...
             ,lit('Agg Unambiguous Residual').alias('rule')\
