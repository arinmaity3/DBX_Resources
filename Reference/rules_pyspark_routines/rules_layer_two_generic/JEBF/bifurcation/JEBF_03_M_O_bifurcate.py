...
def JEBF_03_M_O_bifurcate():
  try:
      ...
      df_input = fin_L1_TMP_JEBifurcation.filter(col('patternID') == lit(patternID))\
                 .select(col('journalSurrogateKey'),col('transactionID')\
                         ,col('transactionIDbyPrimaryKey'),col('transactionLineIDKey')\
                         ,col('transactionLineID'),col('accountID')\
                         ,col('debitCreditCode')\
                         ,expr("case when debitCreditCode = 'C' then creditAmount else debitAmount end").alias("amount"))


      debit_RowNum = Window.partitionBy("transactionID").orderBy("transactionLineIDKey") 
      df_debit = df_input.filter(col('debitCreditCode') == 'D')\
                  .select(col('journalSurrogateKey'),col('transactionLineID')\
                          ,col('transactionIDbyPrimaryKey'), col('transactionID')\
                          ,col('debitCreditCode').alias('debit_code')\
                          ,col('transactionLineID').alias('debit_Line')\
                          ,col('accountID'),col('debitCreditCode')\
                          ,F.row_number().over(debit_RowNum).alias('debit_RowNum')\
      ...
      df1 = df_debit.alias('main')\
              .join(df_credit.alias('sub'),\
              ...
              .select(col('main.journalSurrogateKey')\
                      ...
                      ,concat(col('debit_Line'),lit('.1')).alias('lineItemNumber')\
                      ,concat(col('credit_Line'),lit('.'),col('debit_RowNum')).alias('relatedLineItemNumber'))


      df2 = df_debit.alias('main')\
              .join(df_credit.alias('sub'),\
              (col('main.transactionID') == col('sub.transactionID')),'inner')\
              .select(col('sub.journalSurrogateKey')\
                      ...
                      ,concat(col('credit_Line'),lit('.'),col('debit_RowNum')).alias('lineItemNumber')\
                      ,concat(col('debit_Line'),lit('.1')).alias('relatedLineItemNumber'))
      ...
      fin_L1_STG_JEBifurcation_Link = dfoutput.alias('tTab')\
                                     ...
                                     .select(col('tTab.journalSurrogateKey').alias('journalSurrogateKey')\
                                             ...
                                             ,lit('03-M-O').alias('bifurcationType')\
                                             ,col('jeb1.journalId')\
