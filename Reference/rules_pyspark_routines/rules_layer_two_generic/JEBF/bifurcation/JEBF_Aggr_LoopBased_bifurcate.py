...
def JEBF_Aggr_LoopBased_bifurcate(lsPatterns,ruleSequence):
    try:
      ...
      df_Input = fin_L1_TMP_JEBifurcation_Accountwise_Agg.filter(col('patternID').isin(*lsPatterns))\
             .select(col('journalSurrogateKey') \
              ...
              ,col('debitCreditCode') \
              ,col('patternID') \
              ,expr("cast(case when debitCreditCode = 'C' then creditAmount else debitAmount end as varchar(50))").alias("amount") \
              ,col('accountID'))
      ...
      dfOutputLoop = df_Input.groupby("transactionID","patternID").applyInPandas(ApplyLoopingStrategy,gl_AggLoopingSchema)
      ...
      dfOutputLoop = dfOutputLoop\
            ...
            .withColumn('bifurcationType'\
                  ,expr("case when (count(*) over(partition by transactionID,patternID,oneSideTransactionLineIDKey))=1 \
                              then '11-Agg-MM-O-O' \
                          when oneSideDebitCreditCode = 'D' then '12-Agg-MM-O-M' \
                       else '13-Agg-MM-M-O' end"))
      
      dfOutputLoop = dfOutputLoop.withColumn("lineItemNumber"\
            ,expr("case when bifurcationType <> '11-Agg-MM-O-O' \
      ...
      df_link = dfOutputLoop.alias('trans') \
            .join(fin_L1_TMP_JEBifurcation_Accountwise_Agg.alias('inp'),\
            ...
            .select(col('inp.journalSurrogateKey') \
             ...
             ,lit('Agg Looping Strategy').alias('rule') \
      ...
      objGenHelper.gen_writeToFile_perfom(df=df_link,\
          filenamepath=gl_bifurcationPath+"fin_L1_STG_JEBifurcation_Link.delta",mode="append")
